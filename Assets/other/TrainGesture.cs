        ...
        if ( GameObject.Find("RigidRoundHandDefined(Clone)"))
        {
            ...
            if (getCount == 3)
            {
                ...
                else
                {
                    ...
                    CreateFile(Application.dataPath, "手势数据.txt", "##########################");
                    CreateFile(Application.dataPath, "手势数据.txt", System.DateTime.Now.ToLocalTime().ToString()) ;
                    CreateFile(Application.dataPath, "手势数据.txt", "--------------------------");
                   
                    for (int i = 0; i < data.Count; i++)
                    {
                        CreateFile(Application.dataPath, "手势数据.txt", label[i].ToString()+"    "+data[i].ToString());
                    }
                    total = 0;
                }
